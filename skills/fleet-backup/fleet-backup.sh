...
# Backup agent workspace to its own branch
backup_agent() {
    local agent="$1"
    ...
    # Each agent gets its own branch (e.g., backup-steward, backup-alpha)
    ...
    # Commit and push to agent's own branch
    git commit -m "Fleet backup: ${agent} $(date -Iseconds)"
    git push --force backup "main:${agent_branch}" 2>&1 | tee -a "$BACKUP_LOG"
    
    log "[$agent] Backed up to branch: ${agent_branch}"
}
