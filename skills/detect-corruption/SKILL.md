...
description: Detect data corruption in agent workspace, consensus ledger, and deployment artifacts. Use when the collective needs to verify file integrity, detect tampering, or trigger auto-recovery from corruption events.
---

# Detect Corruption — Integrity Verification

**Purpose:** Detect corruption in triad files, databases, and deployment artifacts.
...
### Main Script: `detect-corruption.sh`
...
```bash
# Scan file integrity
./detect-corruption.sh --scan

# Verify database integrity
./detect-corruption.sh --verify

# Full check (files + DB)
./detect-corruption.sh --full
```
...
- **Triad Resilience:** Corruption recovery
- **Consensus Ledger:** Integrity verification

---

**Corruption detected. Recovery triggered. Integrity restored.** 🦞
